...
EFI_STATUS GetMemoryMap(struct MemoryMap* map) {
    ...
    return gBS->GetMemoryMap(&map->map_size,
                             (EFI_MEMORY_DESCRIPTOR*)map->buffer, &map->map_key,
...
/*gopを取得
gop->Mode->FrameBufferBaseにフレームバッファの先頭アドレス、gop->Mode->FrameBufferSizeに全体のサイズ、
...
void CalcLoadAddressRange(Elf64_Ehdr* ehdr, UINT64* first, UINT64* last) {
    Elf64_Phdr* phdr =
        (Elf64_Phdr*)((UINT64)ehdr +
                      ehdr->e_phoff); /*プログラムヘッダの先頭に合わせる*/
    *first = MAX_UINT64;
    *last = 0;
...
void shell(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE* system_table) {
    EFI_STATUS status;
    ...
    while (TRUE) {
        ...
        /*カーネル*/
            ...
            CalcLoadAddressRange(kernel_ehdr, &kernel_first_addr,
                                 &kernel_last_addr);
            ...
            if (status == EFI_SUCCESS) {
                ...
                UINTN volume_bytes =
                    (UINTN)media->BlockSize * (media->LastBlock + 1);
                if (volume_bytes > 32 * 1024 * 1024) {
                    volume_bytes = 32 * 1024 * 1024;
                }

                Print(
                    L"Reading %lu bytes (Present %d, BlockSize %u, LastBlock "
                    L"%u)\n",
                    volume_bytes, media->MediaPresent, media->BlockSize,
                    media->LastBlock);

                status = ReadBlocks(block_io, media->MediaId, volume_bytes,
                                    &volume_image);
                if (EFI_ERROR(status)) {
