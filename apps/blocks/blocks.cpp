    ...
    for (int by = 0; by < kNumBlocksY; ++by) {
        ...
        for (int bx = 0; bx < kNumBlocksX; ++bx) {
            if (blocks[by][bx]) {
                ...
                WinFillRectangle(layer_id, draw, x, y, kBlockWidth,
                                 kBlockHeight, c);
            }
        }
    }
...
void DrawBar(uint64_t layer_id, int bar_x, bool draw) {
    WinFillRectangle(layer_id, draw, 4 + bar_x, 24 + kBarY, kBarWidth,
                     kBarHeight, 0xffffff);
}

void DrawBall(uint64_t layer_id, int x, int y, bool draw) {
    WinFillRectangle(layer_id, draw, 4 + x - kBallRadius, 24 + y - kBallRadius,
                     2 * kBallRadius, 2 * kBallRadius, 0x007f00);
    WinFillRectangle(layer_id, draw, 4 + x - kBallRadius / 2,
...
extern "C" void main(int argc, char** argv) {
    ...
    const int kBallY =
        kCanvasHeight - kBarFloat - kBarHeight - kBallRadius - 20;
    ...
    for (;;) {
        ...
        WinFillRectangle(layer_id, false, 4, 24, kCanvasWidth, kCanvasHeight,
                         0);
            ...
            if (msg[0].type == Message::kTimerTimeout) {
                ...
                if (!msg[0].arg.keyboard.press) {  // release
                    ...
                    if (bar_x == 0 && move_dir < 0) {
                        move_dir = 0;
                    } else if (bar_x + kBarWidth == kCanvasWidth - 1 &&
                               move_dir > 0) {
                        move_dir = 0;
                    }
                }
        ...
        if (ball_dy < 0 && ball_y_ < kBallRadius) {  // 天井
            ...
            ball_dir = -ball_dir;
                   ...
                   kCanvasHeight - kBallRadius <= ball_y_) {  // 落下
            ball_dir = 0;
            ball_y = -1;
            continue;
        }
