...
struct MemoryMap {
  unsigned long long buffer_size;       /*メモリマップ全体を書き込むためのサイズ*/
  void* buffer;                         /*メモリマップの先頭を指すポインタ*/
  unsigned long long map_size;          /*メモリマップのサイズ*/
  unsigned long long map_key;           /*メモリマップの種類*/
  unsigned long long descriptor_size;   /*個々のEFI_MEMORY_DESCRIPTORのサイズ*/
  uint32_t descriptor_version;          /*EFI_MEMORY_DESCRIPTORのバージョン*/
};
