...
void InitializeMainWindow() {
    main_window = std::make_shared<ToplevelWindow>(160, 68, "Hello Window");
...
extern "C" int main() {
    ...
    while (true) {
        ...
        if (msg[0].type == Message::kMouseMove) {
            ...
            FillRectangle(*layer->GetWindow()->Writer(), {arg.x, arg.y},
                          {arg.w, arg.h}, ToColor(arg.color));
            ...
            WriteString(*layer->GetWindow()->Writer(), {arg.x, arg.y}, &arg.c,
                        ToColor(arg.color));
            if (arg.draw) {
                layer_manager->Draw(arg.layer_id);
            }
            ...
            auto win = layer->GetWindow();
            win->Move(Vector2D<int>{arg.x0, arg.y0},
                      Rectangle<int>{{arg.rx0, arg.ry0}, {arg.rx1, arg.ry1}});
