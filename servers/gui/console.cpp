...
void Console::PutString(const char* s) {
    while (*s) {
        if (*s == '\n') {
            ...
            WriteAscii(*writer_,
                       Vector2D<int>{8 * cursor_column_, 16 * cursor_row_}, *s,
                       fg_color_);
            ++cursor_column_;
        }
        ++s;
...
void Console::Newline() {
    ...
    Rectangle<int> move_src{{0, 16 * (kTitleRows + 1)},
                            {8 * kColumns, 16 * (kRows - kTitleRows - 1)}};
...
void Console::Clear() {
    ...
    FillRectangle(*writer_, {0, 16 * kTitleRows},
                  {8 * kColumns, 16 * (kRows - kTitleRows - 1)}, bg_color_);
    if (layer_manager) {
        layer_manager->Draw(layer_id_);
    }
}
